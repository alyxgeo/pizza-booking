...
import { clearCrat, getCart, getTotalCartPrice } from "../cart/cartSlice";
...
function CreateOrder() {
  ...
  const { username, status: addressStatus, position, address,
    error: errorAddress } = useSelector(state => state.user)
  ...
  return (
    ...
    <div className="px-4 py-6">
      ...
      {/* <Form method="POST" action="/oredr/new"></Form> */}
      <Form method="POST">
        ...
        <div className="mb-12 flex gap-5 items-center">
          <input
            ...
            className="h-6 accent-yellow-400  "
          />
        ...
        <div>
          ...
          <Button disabled={isSubmitting || isLoadingAddress} type='primary'>
            {isSubmitting ? 'Placing Oredr ' : `Order now ${formatCurrency(totalPrice)}`}
          </Button>
        </div>
...
//creating action to mutate data
...
export async function action({ request }) {
  ...
  if (Object.keys(errors).length > 0) return errors
    ...
    //import store to clear cart - do not over use
    store.dispatch(clearCrat())
